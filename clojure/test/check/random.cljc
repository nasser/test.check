...
(defmacro ^:private unsigned-bit-shift-right
  ...
  {:pre [(<= 1 n 63)]}
  (if old-clojure?
    (let [mask (-> #?(:clj Long/MIN_VALUE :cljr Int64/MinValue)
...
(defmacro ^:private longify
  ...
  [num]
  (if (> num #?(:clj Long/MAX_VALUE :cljr Int64/MaxValue))
    (-> num
  ...
  `(-> ~n
       ...
       (as-> z#
           (cond-> z#
             (> 24 (-> z#
                       (bxoubsr 1)
                       #?(:clj (Long/bitCount) :cljr (bit-count2))))
...
(def ^:private golden-gamma
 ...
 (def ^:private next-rng
  ...
  independent results."
   ...
   #?(:clj (let [a (atom (make-java-util-splittable-random (System/currentTimeMillis)))
                 thread-local
                 (proxy [ThreadLocal] []
                   (initialValue []
