...
(def ^:private MAX_INTEGER
  #?(:clj Long/MAX_VALUE :cljr (Int64/MaxValue)))
(def ^:private MIN_INTEGER
  #?(:clj Long/MIN_VALUE :cljr (Int64/MinValue)))

(defn ^:private abs
  [x]
  #?(:clj (Math/abs (long x)) :cljr (Math/Abs x)))

(defn ^:private long->large-integer
  [bit-count x min max]
  (loop [res (-> x
                 (#?(:clj bit-shift-right :cljr bit-shift-right)
                  (- 64 bit-count))
                 ;; -1
                 (cond-> (zero? min) (abs)))]
    (if (<= min res max)
...
(def ^:private POS_INFINITY #?(:clj Double/POSITIVE_INFINITY, :cljr (Double/PositiveInfinity)))
(def ^:private NEG_INFINITY #?(:clj Double/NEGATIVE_INFINITY, :cljr (Double/NegativeInfinity)))
(def ^:private MAX_POS_VALUE #?(:clj Double/MAX_VALUE, :cljr (Double/MaxValue)))
(def ^:private MIN_NEG_VALUE (- MAX_POS_VALUE))
(def ^:private NAN #?(:clj Double/NaN, :cljr Double/NaN))
